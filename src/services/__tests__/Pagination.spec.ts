import faker from 'faker'
import {paginateResponse} from '../Pagination'

describe('Pagination', () => {
  describe('paginateResponse()', () => {
    it('returns a ManyResponse with the first argument as the "data"', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data)

      expect(result).toEqual({
        data,
        count: 1,
        total: 1,
        page: 1,
        pageCount: 1,
      })
    })

    it('handles page input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {page: 2})

      expect(result).toEqual({
        data,
        count: 1,
        total: 1,
        page: 2,
        pageCount: 2,
      })
    })

    it('handles pageSize input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {pageSize: 10})

      expect(result).toEqual({
        data,
        count: 1,
        total: 1,
        page: 1,
        pageCount: 1,
      })
    })

    it('handles total input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {total: 100})

      expect(result).toEqual({
        data,
        count: 1,
        total: 100,
        page: 1,
        pageCount: 1,
      })
    })

    it('handles page & pageSize input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {page: 2, pageSize: 10})

      expect(result).toEqual({
        data,
        count: 1,
        total: 11,
        page: 2,
        pageCount: 2,
      })
    })

    it('handles pageSize & total input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {pageSize: 10, total: 40})

      expect(result).toEqual({
        data,
        count: 1,
        total: 40,
        page: 1,
        pageCount: 4,
      })
    })

    it('handles page, pageSize, and total input', async () => {
      const data = [faker.random.uuid()]
      const result = paginateResponse(data, {
        page: 2,
        pageSize: 10,
        total: 40,
      })

      expect(result).toEqual({
        data,
        count: 1,
        total: 40,
        page: 2,
        pageCount: 4,
      })
    })

    it('handles undefined input', async () => {
      const result = paginateResponse(undefined)

      expect(result).toEqual({
        data: [],
        count: 0,
        total: 0,
        page: 1,
        pageCount: 1,
      })
    })
  })
})
